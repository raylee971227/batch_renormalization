...
class Model(tf.Module):
    def __init__(self, number_classes=NUM_CLASSES, lamda=LAM):
        ...
        x = Activation("relu")(x)
        x = BatchRN(training=True, r_max=2, d_max=1, microbatch_size=64)(x)
        # x = BatchNormalization()(x)

        x = Conv2D(32, (3, 3), padding="same", kernel_regularizer=regularizers.l2(lamda))(x)
        x = Activation("relu")(x)
        x = BatchNormalization()(x)
        x = MaxPooling2D(pool_size=(2, 2))(x)
        x = Dropout(0.3)(x)

        x = Conv2D(64, (3, 3), padding="same", kernel_regularizer=regularizers.l2(lamda))(x)
        x = Activation("relu")(x)
        x = BatchNormalization()(x)
        x = Conv2D(64, (3, 3), padding="same", kernel_regularizer=regularizers.l2(lamda))(x)
        x = Activation("relu")(x)
        ...
        x = Conv2D(128, (3, 3), padding="same", kernel_regularizer=regularizers.l2(lamda))(x)
        x = Activation("relu")(x)
        x = BatchNormalization()(x)
        x = Conv2D(128, (3, 3), padding="same", kernel_regularizer=regularizers.l2(lamda))(x)
        x = Activation("relu")(x)
    ...
    def train(
        ...
        self.model.summary()
        self.model.fit(x=x_train, y=y_train, batch_size=batch_size, epochs=num_epoch, verbose=1)
